...
func updateCaliberPlayerCountChannel(s *discordgo.Session, channelID string, intervalMinutes int) {
	ticker := time.NewTicker(time.Duration(intervalMinutes) * time.Minute)
	defer ticker.Stop()

	for range ticker.C {
		playersCh := make(chan string)
		go scraper.Caliber(playersCh) // Call the scraper in a goroutine (concurrently)
		...
		newName := fmt.Sprintf("Caliber p: %s", players)
